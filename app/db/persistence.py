...
class DatabaseManager:
    ...
    def create_workflow(self, workflow_id: str, repo_url: str, state_json: str) -> None:
        now = datetime.utcnow().isoformat()
        with self._get_connection() as conn:
            conn.execute(
                """
                INSERT INTO workflows (workflow_id, repo_url, status, state_json, created_at, updated_at)
                VALUES (?, ?, 'pending', ?, ?, ?)
            """,
                (workflow_id, repo_url, state_json, now, now),
            )
            conn.commit()
    ...
    def get_workflow_state(self, workflow_id: str) -> Optional[Dict[str, Any]]:
        with self._get_connection() as conn:
            cursor = conn.execute(
                """
                SELECT state_json, status, confidence, reflection_count, current_node
